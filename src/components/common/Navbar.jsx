...
export function Navbar({ activeTab = 'Dashboard', onSelectTab, onOpenCommand }) {
    ...
    <header className="bg-white border-b border-slate-200/80 sticky top-0 z-40 shadow-[0_1px_4px_rgba(0,0,0,0.03)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          {/* Right section: Command shortcut / Notifications & User Profile */}
          <div className="flex items-center gap-2 sm:gap-4">
            {/* Quick Command Palette Button (Desktop / Tablet) */}
            {onOpenCommand && (
              <>
                <button
                  type="button"
                  onClick={onOpenCommand}
                  className="hidden md:inline-flex items-center gap-2 px-2.5 py-1.5 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100 text-slate-500 hover:text-slate-700 text-xs font-semibold transition cursor-pointer"
                ...
                <button
                  type="button"
                  onClick={onOpenCommand}
                  className="inline-flex md:hidden p-2 text-slate-600 hover:text-slate-900 rounded-full hover:bg-slate-100 transition cursor-pointer"
                  title="Quick Actions"
                >
            ...
            <div className="relative">
              ...
              {showNotifications && (
                <div className="absolute right-0 sm:right-0 -mr-12 sm:mr-0 mt-2 w-72 sm:w-80 max-w-[calc(100vw-2rem)] bg-white rounded-2xl shadow-xl border border-slate-100 py-3 z-50 text-left">
                  ...
                  <div className="divide-y divide-slate-50">
                    <div className="px-4 py-3 hover:bg-slate-50 cursor-pointer">
                      <p className="text-xs font-semibold text-slate-800">
                        New perk claimed at The Daily Grind
                      </p>
                      <p className="text-[11px] text-slate-400 mt-0.5">
                        2 min ago • R5.00 Staff Fund added
                      </p>
                    </div>
                    <div className="px-4 py-3 hover:bg-slate-50 cursor-pointer">
                      <p className="text-xs font-semibold text-slate-800">
            ...
            <div className="px-3 py-1 mb-2 flex items-center justify-between">
              ...
              {onOpenCommand && (
                <button
                  ...
                  className="text-xs font-semibold text-orange-600 bg-orange-50 px-2.5 py-1 rounded-lg hover:bg-orange-100"
                >
                  ⌘ Quick Actions
                </button>
              )}
